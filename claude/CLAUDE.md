...
- Discussion is not authorization; wait for an explicit instruction to implement.
- Once instructed, implement fully without asking for further confirmation.
...
- When asked for a plan, state open questions, ambiguities and likely sources
  of issues.
- Stop and ask if work would drift beyond the agreed scope, or if something
  unplanned emerges during implementation.
