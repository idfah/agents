...
Use custom subagent configurations for these default roles:
...
custom agent when available and a generic subagent only when necessary.

Launch all subagents in a single batch so they run in parallel, with no
inherited conversation context.  Give each the same core brief plus its
role-specific perspective.  Tell each agent to work independently, not
delegate and not consult other agents.
